...
#extend data
flags.DEFINE_string('image_extend_dir', '', 'Directory containing images.')
flags.DEFINE_string(
    'object_extend_annotations_file', '', 'File containing object '
                                          'annotations - boxes and instance masks.')
FLAGS = flags.FLAGS
...
def create_tf_example(image,
    ...
    Args:
      image_extend_dir: extend image data dir
    ...
    full_path = None
    if os.path.isfile(os.path.join(image_dir, 'images/traffic_train', filename)):
        full_path = os.path.join(image_dir, 'images/traffic_train', filename)
    elif os.path.isfile(os.path.join(image_extend_dir, 'images', filename)):
        full_path = os.path.join(image_extend_dir, 'images', filename)
...
def _load_object_annotations(object_annotations_file, object_extend_annotations_file=None):
    """Loads object annotation JSON file."""
    with tf.io.gfile.GFile(object_annotations_file, 'r') as fid:
        obj_annotations = json.load(fid)
        # print(obj_annotations)
    print("Loaded annotations from original datasets with length {}".format(len(obj_annotations['images'])))

    """Loads extend object annotation JSON file."""
    if object_extend_annotations_file:
        print(object_extend_annotations_file)
        with tf.io.gfile.GFile(object_extend_annotations_file, 'r') as fid:
            obj_extend_annotations = json.load(fid)
            # print(obj_extend_annotations)
            obj_annotations['images'] += obj_extend_annotations['images']
            obj_annotations['annotations'] += obj_extend_annotations['annotations']
        print("Loaded annotation from extend dataset with length {}".format(len(obj_annotations['images'])))

    images = obj_annotations['images']
    category_index = label_map_util.create_category_index(
        obj_annotations['categories'])
    print(len(images), len(obj_annotations['annotations']))
...
def _create_tf_record_from_coco_annotations(image_info_file,
                                            ...
                                            object_extend_annotations_file=None,
                                            include_masks=False):
    ...
    if object_annotations_file:
        img_to_obj_annotation, category_index = (
            _load_object_annotations(object_annotations_file, object_extend_annotations_file))
    ...
    for idx, (_, tf_example, num_annotations_skipped) in enumerate(
            pool.imap(
                ...
                [(image, image_dir, image_extend_dir, _get_object_annotation(image['id']),
...
def main(_):
    ...
    _create_tf_record_from_coco_annotations(image_info_file, FLAGS.image_dir,
                                            FLAGS.image_extend_dir,
                                            FLAGS.output_file_prefix,
                                            FLAGS.num_shards,
                                            FLAGS.object_annotations_file,
                                            FLAGS.object_extend_annotations_file,
                                            FLAGS.include_masks)
